...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    while (1)
    {
        ...
        // Only act if the button state just changed!
        if (current_button_state != previous_button_state)
        {
            if(current_button_state == GPIO_PIN_RESET)
            {
                ...
                // Send UART message
                char msg[] = "Button Pressed! LED OFF\r\n";
            ...
            else
            {
                ...
                // Send UART message
                char msg[] = "Button Released! LED ON\r\n";
                HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            }

            // Small 50ms delay for debouncing (prevents physical button springs from sending double-signals)
            HAL_Delay(50);
        }
